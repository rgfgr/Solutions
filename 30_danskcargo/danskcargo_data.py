...
class Container(Base):
    ...
    def convert_from_tuple(tuple_):
        try:
            ...
            weight = int(tuple_[1])
            if weight < 0:
                messagebox.showwarning("", "Weight must not be negative!")
            else:
                container = Container(id=id_, weight=weight, destination=tuple_[2])
                return container
        except Exception as error:
            messagebox.showwarning(error.__repr__(), "Entries could not be converted to container!")
...
class Aircraft(Base):
    ...
    def convert_from_tuple(tuple_):
        try:
            ...
            messagebox.showwarning(error.__repr__(), "Entries could not be converted to aircraft!")
...
class Transport(Base):
    ...
    def convert_from_tuple(tuple_):
        try:
            ...
            if container_id < 0:
                messagebox.showwarning("", "Container id must not be negative!")
                return
            if aircraft_id < 0:
                messagebox.showwarning("", "Aircraft id must not be negative!")
                return
            transport = Transport(id=id_, date=date, container_id=container_id, aircraft_id=aircraft_id)
            return transport
        except Exception as error:
            messagebox.showwarning(error.__repr__(), "Entries could not be converted to transport!")
